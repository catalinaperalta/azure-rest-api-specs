 ...
 */
export async function getRepoRoot(anyPathInRepo) {
 ...
 */
export async function resolveCommitish(commitish, repoPath) {
    const dir = await directoryOf(repoPath);
    const { stdout } = await execFileAsync("git", ["rev-parse", "--verify", `${commitish}^{commit}`], { cwd: dir });
    return stdout.trim();
}
/**
 ...
 */
export async function checkoutRevision(commitish, repoPath, options = {}) {
    ...
    try {
        ...
        throw new Error(`Failed to check out revision "${commitish}" (resolved to ${sha}) as a git worktree: ` +
 ...
 */
export async function mapPathIntoWorktree(originalPath, repoPath, worktreePath) {
    const repoRoot = await getRepoRoot(repoPath);
 ...
 */
async function linkNodeModulesIfPresent(repoRoot, worktreeRoot) {
...
async function directoryOf(path) {
